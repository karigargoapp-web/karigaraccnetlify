...
import { IoTrendingUp, IoCash, IoPricetag, IoCalendar } from 'react-icons/io5'
...
export default function AdminRevenue() {
  ...
  const fetchRevenue = useCallback(async () => {
    ...
    if (from) {
      commQ = commQ.gte('created_at', from)
      feeQ = feeQ.gte('created_at', from)
      jobQ = jobQ.gte('completed_at', from)
    }
    ...
    comms?.forEach(r => {
      ...
      grouped[date] = (grouped[date] || 0) + r.amount
    })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Platform earnings breakdown</p>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Bidding Fees', value: `₨${stats.biddingFees.toLocaleString()}`, icon: IoPricetag, color: 'text-blue-600', bg: 'bg-blue-50' },
              { label: 'Jobs Completed', value: stats.jobsCompleted, icon: IoCalendar, color: 'text-purple-600', bg: 'bg-purple-50' },
            ].map((s, i) => (
                ...
                {dailyRevenue.map((d, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <div className="w-full bg-primary rounded-t-sm transition-all"
                      style={{ height: `${Math.max((d.amount / maxBar) * 120, 4)}px` }} />
                ...
                {topJobs.map(j => (
                  <div key={j.id} className="px-6 py-4 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{j.customer_name} → {j.worker_name}</p>
                    </div>
